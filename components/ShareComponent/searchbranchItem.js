...
import { getNodesByLine } from '../../api/line';
...
export default class SearchBranchItem extends React.Component {
  constructor(props) {
    ...
    this.state = {owner: '', re: false, copied: false, task: [], open: false};
  ...
  render() {
    ...
    const taskList = this.state.task.map((user) => {
      return <li key={user._id} className='ring-2 ring-gray-200 rounded-lg mx-2 px-4 sm:text-sm text-xs py-1 items-center'>{user.title}</li>;
    });
    return (
      <>
        <div className='container shadow rounded-lg p-3 py-5 px-4 my-3 flex-col flex items-center cursor-pointer bg-white'>
          ...
          <ul className='flex-row flex justify-start container my-3 overflow-scroll py-1 items-center'>
            {taskList}
          </ul>
        </div>
      </>
